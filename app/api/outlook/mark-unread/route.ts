...
export async function POST(req: NextRequest) {
  ...
  const tokenResult = await getOutlookAccessToken(userId, "mark-unread", requestedAccountId)
  if (!tokenResult.ok) {
  ...
  try {
    ...
    return NextResponse.json({ error: err.message || "Failed to mark conversation unread" }, { status: 500 })
  }
}
