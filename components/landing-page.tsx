...
const HERO_SIDEBAR_ICONS = [
  { Icon: IconSearch, active: false },
  { Icon: IconMail, active: false },
  { Icon: IconUsers, active: false },
  { Icon: IconGitBranch, active: true },
  { Icon: IconCircleCheck, active: false },
  { Icon: IconBuildingStore, active: false },
  { Icon: IconChartBar, active: false },
]
        ...
        <div className={styles.heroRight}>
          <div className={styles.heroMockup}>
            <div className="h-full w-full flex font-[Inter,system-ui,-apple-system,sans-serif] overflow-hidden">
              {/* Sidebar */}
              <div className="w-[84px] shrink-0 bg-[#0F6B3E] flex flex-col items-center pt-3 pb-2 gap-3">
                <Image src="/INSTROOM WHITE.png" alt="Instroom" width={56} height={12} className="object-contain" />
                <div className="flex flex-col items-center gap-2 mt-1">
                  {HERO_SIDEBAR_ICONS.map(({ Icon, active }, i) => (
                    <div
                      key={i}
                      className={`w-7 h-7 rounded-lg flex items-center justify-center ${active ? "bg-[#1FAE5B]" : ""}`}
                    >
                      <Icon size={14} stroke={1.75} className="text-white" />
                    </div>
                  ))}
                  ...
                  {HERO_PIPELINE_COLUMNS.map((col) => (
                    <div key={col.key} className="flex flex-col gap-1 flex-1 min-w-0">
                      <div className={`rounded-[5px] px-1.5 py-1 text-[6.5px] font-bold flex items-center justify-between gap-1 ${col.headerClass}`}>
                        ...
                        <span className={`rounded-full px-1 shrink-0 ${col.badgeClass}`}>{col.count}</span>
                      </div>
