...
function EmailModal({
  ...
  const handleSend = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(provider === "outlook" ? "/api/outlook/send" : "/api/gmail/send", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ to: to.trim(), subject: subject.trim() || "(No subject)", body: body.trim() }),
      })
            ...
            {(["gmail", "outlook"] as const).map((p) => (
              <button
                ...
                style={{
                  ...
                  border: provider === p ? "1.5px solid #1fae5b" : "1px solid #e5e7eb",
                  background: provider === p ? "#f0fdf4" : "#fff",
                  color: provider === p ? "#15803d" : "#6b7280",
                }}
              >
